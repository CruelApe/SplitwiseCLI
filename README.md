...
- **`dotnet tool install` (Option A):** `update` still won't directly touch a tool install's files (they live in the NuGet tool store, not somewhere this command should manage) — but if it's run from **inside your local clone** of this repo, it detects that and runs `git pull` + `dotnet pack` + `dotnet tool update --global` for you automatically (each step's output is shown, and it stops at the first one that fails without touching the rest). Run it from anywhere else and it just prints those same three commands for you to run by hand instead.
